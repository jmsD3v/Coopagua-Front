...
export default function UsersPage() {
  ...
  if (error) return <div>Error al cargar los usuarios. Puede que no tengas permisos de administrador.</div>;
  if (isLoading) return <div>Cargando...</div>;

  if (!Array.isArray(users)) {
    if (users && typeof users === 'object' && 'error' in users) {
      return <div>Error al cargar usuarios: {users.error}</div>;
    }
    return <div>Error: Se esperaba una lista de usuarios. Revisa la consola.</div>;
  }
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      <div className="sm:flex sm:items-center">
        <div className="sm:flex-auto">
          <h1 className="text-2xl font-bold leading-6">Gestión de Usuarios</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            Una lista de todos los socios y empleados de la cooperativa.
          </p>
        </div>
        <div className="mt-4 sm:ml-16 sm:mt-0 sm:flex-none">
          <Button asChild>
            <Link href="/dashboard/users/new">Añadir usuario</Link>
          </Button>
        </div>
      </div>
